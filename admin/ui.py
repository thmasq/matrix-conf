...
class ScreenManager:
    ...
    def pause_for_input(self, message: str = "Press Enter to continue...") -> None:
...
class TerminalPaginator:
    ...
    def show_navigation_help(self) -> None:
        ...
        print(
            f"  Page {self.current_page + 1} of {self.total_pages} ({len(self.items)} total items)",
        )
        ...
        while True:
            try:
                ...
                if choice in {"", "n", "next"}:
                    ...
                    try:
                        page_num = (
                            int(input(f"Go to page (1-{self.total_pages}): ")) - 1
                        )
...
class FilterSortUI:
    ...
    @staticmethod
    def show_filter_sort_status(
        current_filter: str,
        current_filter_type: str,
        current_sort: str,
        total_items: int,
        filtered_count: int,
        item_type: str = "items",
        ...
        if current_filter:
            if current_filter_type:
                ...
                filter_type_display = filter_type_names.get(
                    current_filter_type,
                    current_filter_type,
                )
    ...
    def handle_pagination_navigation(choice: str, paginator: TerminalPaginator) -> bool:
        ...
        if choice in {"g", "goto"}:
            if paginator.needs_pagination():
                try:
                    page_num = (
                        int(input(f"Go to page (1-{paginator.total_pages}): ")) - 1
                    )
    ...
    @staticmethod
    def show_navigation_options(has_pagination: bool, has_items: bool) -> None:
        """Show available navigation options."""
        if has_pagination and has_items:
            print("\nNavigation:")
            print("  [Enter] Next page  [p] Previous page  [g] Go to page")
            print(
                "  [f] Filter  [s] Sort  [c] Clear filter  [r] Reset  [q] Cancel/Quit",
            )
        else:
            print("\nOptions:")
            print(
